...
function attRender() {
  ...
  el.innerHTML = days.map((d, i) =>
    '<div class="att-day">' +
      ...
      '<div class="att-stamp' + (d.done ? ' done' : '') + (d.isToday && !d.done ? ' today' : '') + '">' +
        (d.done ? STAMPS[i] : (d.isToday ? '📌' : '○')) +
      '</div>' +
    '</div>'
  ).join('');
  // 오늘 출석 체크
...
function switchTab(name,btn){
  // 설정탭: 관리자 로그인 필요
  if(name==='settings' && !adminIsLogged()){
    document.querySelectorAll('.tab').forEach(t=>t.classList.remove('on'));
    if(btn) btn.classList.add('on');
    showToast('🔒 설정은 관리자만 사용할 수 있어요!');
    adminLogin();
    return;
  }
  // 퀴즈 진행 중 타이머 일시정지/재개
...
function restartQuiz(){
  ...
  stopTimer();S.quizStarted=false; _setSbarExam(false);
...
function initQuizState(customPool){
  ...
  Object.assign(S,{queue,idx:0,score:0,combo:0,maxCombo:0,correct:0,wrong:0,lives:MAX_LIVES,locked:false,gameOver:false,timeLeft:TIME_NOW,catCorrect:{},catWrong:{}});
  // 타이머 제한시간도 업데이트
  S._TIME=TIME_NOW;
...
function ans(v){
  ...
  if(ok){
    ...
    S.wrong++;S.combo=0;S.lives--;S.catWrong[cat]++;
...
function renderWrongNote(){
  ...
  EL.wnList.innerHTML=list.map((w,i)=>{
    ...
    return`<div class="wn-item">
      ...
      <button class="wn-del" onclick="deleteWrong(${i})" title="삭제">🗑</button>
    </div>`;
  }).join('');
}
function deleteWrong(i){
  const list=loadWrong();list.splice(i,1);
  lsSet(WRONG_KEY, list);
  renderWrongNote();updateWrongBtn();
}
...
function ytDoSearch(q){
  ...
  fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=6&type=video&q=${sq}&key=${apiKey}`)
    .then(r=>r.json())
    .then(data=>{
      if(data.error){
      ...
      let html = `<div style="background:#fff;border:2px solid var(--border);border-radius:14px;overflow:hidden;">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 16px;background:linear-gradient(135deg,#ff0000,#cc0000);">
          <span style="font-size:14px;font-weight:900;color:#fff;font-family:Jua,sans-serif;">📺 "${q}" 검색 결과</span>
      ...
      data.items.forEach(item=>{
        ...
        html += `<div onclick="ytLoad('https://www.youtube.com/watch?v=${vid}')"
          ...
          <div style="font-size:11px;color:#fff;background:#ff0000;border-radius:6px;padding:3px 8px;white-space:nowrap;flex-shrink:0;align-self:flex-start;">▶ 재생</div>
        </div>`;
      });
      html += `</div></div>`;
      results.innerHTML = html;
    })
    .catch(()=>{ results.innerHTML = ytSearchNoKeyUI(q, sq, '⚠️ 네트워크 오류'); });
}
  ...
  return `<div style="background:#fff;border:2px solid var(--border);border-radius:14px;overflow:hidden;">
    ...
    <div style="padding:16px;">
      ...
      <div style="font-size:13px;color:#4a5568;margin-bottom:12px;line-height:1.6;">
        앱 안에서 유튜브 검색 결과를 보려면 <b>YouTube Data API v3 키</b>가 필요해요.<br>
        아래에 입력하면 이 기기에 저장되고, 이후 자동으로 검색 결과가 표시돼요!
      </div>
      <div style="background:#f8fafc;border:1.5px solid var(--border);border-radius:10px;padding:12px 14px;margin-bottom:12px;font-size:12px;color:#64748b;line-height:1.7;">
        📌 API 키 발급 방법:<br>
        1. <a href="https://console.cloud.google.com/" target="_blank" style="color:#f06878;">Google Cloud Console</a> 접속<br>
        2. 새 프로젝트 생성 → API 및 서비스 → 사용자 인증 정보<br>
        3. API 키 만들기 → YouTube Data API v3 사용 설정
      </div>
...
function soopDoSearch(q){
  ...
  const sq = encodeURIComponent(q);
  // SOOP 검색 URL (방송인 검색)
  const soopSearchUrl = 'https://www.sooplive.co.kr/search?szLocation=main&szSearchType=bj&szSearchWord='+sq;
  results.innerHTML = `
...
function mpUpdateProgress(){
  if(!MP.audio)return;
  const d=MP.audio.duration||0,c=MP.audio.currentTime||0;
...
function adminRefreshOverlay() {
  ...
  if (typeof S !== 'undefined' && cqList) {
    ...
    cqList.innerHTML = S.customQs.length
      ? S.customQs.map((q,i) => '<div class="admin-cq-item"><span style="flex:1">' + q[0] + '</span><button class="admin-cq-del" onclick="adminDelCQ(' + i + ')">🗑</button></div>').join('')
  ...
  grid.innerHTML = list.slice().reverse().map((n, ri) => {
    ...
    return '<div class="ng-card' + (n.read ? '' : ' unread') + '" onclick="noticeOpen(' + i + ')">' +
      ...
      '<div class="ng-card-body">' +
        ...
        '<div class="ng-card-title">' + n.title + '</div>' +
        '<div class="ng-card-meta">' +
          '<span class="ng-card-from">' + (n.from || '담임교사') + '</span>' +
          '<span class="ng-card-date">' + n.date + '</span>' +
        '</div>' +
      '</div>' +
    '</div>';
  }).join('');
...
function soopAdd() {
  ...
  val.split(',').map(x=>x.trim()).filter(Boolean).forEach(id => {
...
function snakePause(){
  ...
  if(SNAKE.paused){SNAKE.paused=false;snakeLoop();}
